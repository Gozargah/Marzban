                ...
                <ChartContainer config={chartConfig}>
                  <ResponsiveContainer width="100%" height={width < 500 ? 200 : 320}>
                    ...
                    >
                      ...
                      />
                      <RechartsPrimitive.Bar dataKey="usage" radius={6}>
                        {chartData.map((_: { period: string; usage: number }, index: number) => (
                          <RechartsPrimitive.Cell key={`cell-${index}`} fill={index === activeIndex ? 'hsl(var(--muted-foreground))' : 'hsl(var(--primary))'} />
                        ))}
                        {chartData.length <= 7 && (
                          <RechartsPrimitive.LabelList dataKey="usage" position="top" offset={8} className="fill-foreground" fontSize={12} formatter={(value: number) => value.toFixed(2)} />
                        )}
