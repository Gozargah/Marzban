# Contribute to NeonGate
Thanks for considering contributing to NeonGate!
...
- Also tell the version of NeonGate, Xray and docker (if you use docker) you are using.
...
## NeonGate CLI
NeonGate CLI (`marzban-cli` binary) is built using [Typer](https://typer.tiangolo.com/), and its commands' code can be found in `cli`  directory. Its documentation is generated using [Typer CLI](https://typer.tiangolo.com/typer-cli/) which can be re-generated by navigating to project's root directory and running the following command (`typer-cli` package needs to be installed first):
