  ...
  return isOpen ? (
    <Collapsible open={isCollapsed} onOpenChange={setIsCollapsed} className="w-full">
      <CollapsibleTrigger className="[&[data-state=open]>div>div>svg]:rotate-180 mb-1" asChild>
        <Button variant={isSubmenuActive ? 'secondary' : 'ghost'} className="w-full justify-start h-9 px-3">
          <div className={cn('w-full flex-row items-center flex justify-between', isRTL && 'flex-row-reverse')}>
            <div className={cn('flex items-center', isRTL && 'flex-row-reverse')}>
              <span className={cn(isRTL ? 'ml-3' : 'mr-3')}>
                <Icon size={18} />
              </span>
              <p className={cn('max-w-[150px] truncate', isOpen ? 'translate-x-0 opacity-100' : '-translate-x-96 opacity-0')}>{t(label)}</p>
            </div>
      ...
      <CollapsibleContent className="overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down">
        {submenus.map(({ href, label, active }, index) => (
          <SidebarMenuSub key={index} className={cn(isRTL && 'border-r border-l-0')}>
            <Button variant={(active === undefined && pathname === href) || active ? 'secondary' : 'ghost'} className="w-full justify-start h-9 mb-1" asChild>
              <Link className={cn(isRTL && 'flex-row-reverse')} to={href}>
                <p className={cn('max-w-[170px] truncate', isOpen ? 'translate-x-0 opacity-100' : '-translate-x-96 opacity-0')}>{t(label)}</p>
              </Link>
            </Button>
          </SidebarMenuSub>
        ))}
      ...
      <DropdownMenuContent side="right" sideOffset={25} align="start">
        ...
        {submenus.map(({ href, label, active }, index) => (
          <DropdownMenuItem key={index} asChild>
            <Link className={`cursor-pointer ${((active === undefined && pathname === href) || active) && 'bg-secondary'}`} to={href}>
