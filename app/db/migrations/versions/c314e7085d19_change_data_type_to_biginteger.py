...
def upgrade() -> None:
    ...
    conn = op.get_bind()
    conn.execute('UPDATE user_templates SET data_limit_new = data_limit')
    conn.execute('UPDATE user_templates SET expire_duration_new = expire_duration')
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('user_templates', sa.Column('data_limit_new', sa.Integer()))
    op.add_column('user_templates', sa.Column('expire_duration_new', sa.Integer()))
    conn = op.get_bind()
    conn.execute('UPDATE user_templates SET data_limit_new = data_limit')
    conn.execute('UPDATE user_templates SET expire_duration_new = expire_duration')
    op.drop_column('user_templates', 'data_limit')
