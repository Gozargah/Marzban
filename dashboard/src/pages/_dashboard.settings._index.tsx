...
import { useForm as useWebhookForm, useFieldArray as useWebhookFieldArray, Controller } from 'react-hook-form'
              ...
              <CardContent className="space-y-6">
                <form onSubmit={handleSubmit(onSubscriptionSubmit)} className="space-y-4">
                  <div>
                    ...
                    {errors.url_prefix && <span className="text-red-500 text-xs">{errors.url_prefix.message}</span>}
                  </div>
                  <div>
                    <Label htmlFor="update_interval">Automatic update interval</Label>
                    <Input id="update_interval" type="number" {...register('update_interval', { valueAsNumber: true })} placeholder="1" />
                    {errors.update_interval && <span className="text-red-500 text-xs">{errors.update_interval.message}</span>}
                  </div>
                  <div>
                    <Label htmlFor="profile_title">Page Title</Label>
                    <Input id="profile_title" {...register('profile_title')} placeholder="Marzban" />
                    {errors.profile_title && <span className="text-red-500 text-xs">{errors.profile_title.message}</span>}
                  </div>
                  <div>
                    <Label htmlFor="support_url">Support URL</Label>
                    <Input id="support_url" {...register('support_url')} placeholder="https://..." />
                    {errors.support_url && <span className="text-red-500 text-xs">{errors.support_url.message}</span>}
                  </div>
                  <div className="flex items-center justify-end my-4">
                    <Button type="submit" disabled={subscriptionLoading}>
                      {subscriptionLoading && <Loader2 className="animate-spin w-4 h-4 mr-2 inline" />}
            ...
            <Card className="flex-1 pt-6 w-full">
              ...
              <CardContent className="space-y-6">
                <form onSubmit={telegramForm.handleSubmit(onTelegramSubmit)} className="space-y-4">
                  <div className="flex items-center w-full justify-between">
                    <div className="space-y-2 flex-1">
                      <Label className="text-base" htmlFor="telegram-token">API Token</Label>
                    </div>
                    <div className="flex-1">
                      <Input id="telegram-token" {...telegramForm.register('token')} placeholder="" />
                    </div>
                  </div>
                  <div className="flex items-center w-full justify-between">
                    <div className="space-y-2 flex-1">
                      <Label className="text-base" htmlFor="telegram-webhook-url">Webhook URL</Label>
                    </div>
                    <div className="flex-1">
                      <Input id="telegram-webhook-url" {...telegramForm.register('webhook_url')} placeholder="" />
                    </div>
                  </div>
                  <div className="flex items-center w-full justify-between">
                    <div className="space-y-2 flex-1">
                      <Label className="text-base" htmlFor="telegram-webhook-secret">Webhook Secret</Label>
                    </div>
                    <div className="flex-1">
                      <Input id="telegram-webhook-secret" {...telegramForm.register('webhook_secret')} placeholder="" />
                    </div>
                  </div>
                  <div className="flex items-center w-full justify-between">
                    <div className="space-y-2 flex-1">
                      <Label className="text-base" htmlFor="telegram-proxy-url">Proxy URL</Label>
                    </div>
                    <div className="flex-1">
                      <Input id="telegram-proxy-url" {...telegramForm.register('proxy_url')} placeholder="" />
                    </div>
                  </div>
                  <div className="flex items-center justify-end my-4">
                    <Button type="submit" disabled={telegramLoading}>
                      {telegramLoading && <Loader2 className="animate-spin w-4 h-4 mr-2 inline" />}
                ...
                <form onSubmit={discordForm.handleSubmit(onDiscordSubmit)} className="space-y-4">
                  <div className="flex items-center justify-end my-4">
                    <Button type="submit" disabled={discordLoading}>
                      {discordLoading && <Loader2 className="animate-spin w-4 h-4 mr-2 inline" />}
                  ...
                  <div>
                    ...
                    {webhooksFieldArray.fields.map((field, idx) => (
                      <div key={field.id} className="flex gap-2 mb-2">
                        <Input placeholder="URL" {...webhookForm.register(`webhooks.${idx}.url`)} />
                        <Input placeholder="Secret" {...webhookForm.register(`webhooks.${idx}.secret`)} />
                        <Button type="button" variant="destructive" onClick={() => webhooksFieldArray.remove(idx)}>-</Button>
                      </div>
                    ))}
                    <Button type="button" onClick={() => webhooksFieldArray.append({ url: '', secret: '' })}>Add Webhook</Button>
                  </div>
                  <div>
                    <Label className="text-base" htmlFor="webhook-timeout">Timeout</Label>
                    <Input id="webhook-timeout" type="number" {...webhookForm.register('timeout')} placeholder="180" />
                  </div>
                  <div>
                    <Label className="text-base" htmlFor="webhook-recurrent">Recurrent</Label>
                    <Input id="webhook-recurrent" type="number" {...webhookForm.register('recurrent')} placeholder="3" />
                  </div>
                  <div>
                    <Label className="text-base" htmlFor="webhook-proxy-url">Proxy URL</Label>
                    <Input id="webhook-proxy-url" {...webhookForm.register('proxy_url')} placeholder="" />
                  </div>
                  <div className="flex items-center justify-end my-4">
                    <Button type="submit" disabled={webhookLoading}>
                      {webhookLoading && <Loader2 className="animate-spin w-4 h-4 mr-2 inline" />}
