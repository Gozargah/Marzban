...
import { useCreateNode, useModifyNode, NodeConnectionType, useGetAllCores, CoreResponse, getNode, reconnectNode, useSyncNode } from '@/service/api'
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={onOpenChange}>
      <DialogContent className="h-full max-w-full sm:max-w-[90vw] lg:h-auto lg:max-w-[1000px]" onOpenAutoFocus={e => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className={cn('text-start text-xl font-semibold', dir === 'rtl' && 'sm:text-right')}>{editingNode ? t('editNode.title') : t('nodeModal.title')}</DialogTitle>
          <p className={cn('text-start text-sm text-muted-foreground', dir === 'rtl' && 'sm:text-right')}>{editingNode ? t('nodes.prompt') : t('nodeModal.description')}</p>
        </DialogHeader>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              {editingNode && (
                <div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap sm:items-center sm:gap-2">
                  <Button variant="outline" size="sm" onClick={() => setShowOnlineStats(true)} disabled={syncing || reconnecting} className="h-7 flex-shrink-0 px-2 text-xs">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col">
            <div className="-mr-2 max-h-[49dvh] overflow-y-auto px-1 pr-2 sm:-mr-4 sm:max-h-[65dvh] sm:px-2 sm:pr-4">
            ...
            <div className="flex flex-col justify-end gap-2 pt-4 sm:flex-row">
              <Button variant="outline" onClick={() => onOpenChange(false)} disabled={addNodeMutation.isPending || modifyNodeMutation.isPending} className="w-full sm:w-auto">
                {t('cancel')}
              </Button>
              <LoaderButton
                type="submit"
      ...
      <UserOnlineStatsDialog isOpen={showOnlineStats} onOpenChange={setShowOnlineStats} nodeId={editingNodeId || null} nodeName={form.getValues('name')} />
    </Dialog>
  )
}
