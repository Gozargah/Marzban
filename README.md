## Форк Bourbon24k (yuku)

Продакшен-форк Marzban `v0.8.4` для коммерческого VPN-сервиса. Добавляет то, чего не хватает
стоковой панели при реальной эксплуатации: обход блокировок через CDN, лимиты по устройствам и
группам хостов, понятные уведомления клиенту, расширенный дашборд и устойчивость под нагрузкой.
...
- XHTTP GET-uplink (поля `uplinkHTTPMethod`, `xPadding*`, `scStreamUpServerSecs`) сохраняются в
  подписках и v2ray-json — работает обход БС через Yandex Cloud CDN.
- Ноды устойчивее проходят health-check (таймауты 3 → 15 c, корректная обработка API-timeout).

**Лимиты и устройства**
- **Лимит устройств по HWID** — регистрация устройств из заголовков подписки (`x-hwid`,
  модель, ОС, версия, user-agent), подсчёт и ограничение. Неизвестные клиенты (без HWID)
  отслеживаются по user-agent — закрыт обход через v2rayNG.
- **Отзыв (soft-ban) устройства** — устройство остаётся в истории, но блокируется.
- **Лимит трафика по группам хостов** — отдельный счётчик трафика на именованную группу
  нод/хостов с per-user переопределением; остаток показывается прямо в названии сервера
  (`🇷🇺Россия LTE (1.2/5.0 ГБ)`) и обновляется. Поддержка мастер-ноды.
- Истёкшие/превысившие лимит видят **сервер-уведомление** в клиенте вместо мёртвых серверов.

**Дашборд**
- Расширенная статистика: онлайн за 24ч, трафик, live-скорость, CPU, память — карточками.
- Блок «Статистика устройств»: активные/отозванные, юзеры с лимитом/сверх лимита, график по
  платформам.
- Карточка устройства с полной информацией (HWID, модель, ОС, user-agent, первый/последний вход)
  и кнопками отзыва/удаления.
- Модалка **«YUKU настройки»**: тексты уведомлений и лимит устройств по умолчанию из UI.
- Почти чёрная тёмная тема.

**Надёжность**
- SQLite в режиме WAL (`busy_timeout`, `synchronous=NORMAL`) — нет «database is locked» под
  параллельной нагрузкой на `/sub`.
- Ограниченный пул потоков для операций с xray (не плодит потоки на каждый запрос).

<p align="center">
  <img src="docs/screenshots/dashboard.png" alt="Дашборд: статистика и устройства" width="900">
</p>
<p align="center">
  <img src="docs/screenshots/user-devices.png" alt="Карточка пользователя с устройствами" width="430">
  <img src="docs/screenshots/yuku-settings.png" alt="YUKU настройки" width="430">
</p>
