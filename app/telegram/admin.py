...
from app.utils.xray import (
    xray_add_user,
    xray_config_include_db_clients,
    xray_remove_user,
)
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("delete:"), is_admin=True
)
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("users:"), is_admin=True
)
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("user:"), is_admin=True
)
...
└ Subscription URL: <code>{url}</code>
        ...
        expire_date=datetime.fromtimestamp(user.expire).strftime("%Y-%m-%d")
        if user.expire
        else "Never",
...
def add_user_username_step(message: types.Message):
    if message.text == "Cancel":
        return bot.send_message(
            message.chat.id, "✅ Cancelled.", reply_markup=BotKeyboard.main_menu()
        )
    if not message.text:
...
def add_user_expire_step(message: types.Message, username: str, data_limit: int):
    ...
    bot.send_message(
        ...
        "Select Protocols:\nUsernames: {}\nData Limit: {}\nExpiry Date {}".format(
            ...
            readable_size(mem_store.get("data_limit"))
            if mem_store.get("data_limit")
            else "Unlimited",
            mem_store.get("expire_date").strftime("%Y-%m-%d")
            if mem_store.get("expire_date")
            else "Never",
        ),
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("select:"), is_admin=True
)
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("confirm:"), is_admin=True
)
def confirm_user_command(call: types.CallbackQuery):
    ...
    if data == "delete":
        ...
        if not mem_store.get("protocols"):
            return bot.answer_callback_query(
                call.id, "❌ No protocols selected.", show_alert=True
            )
...
""".format(
        ...
        bot.edit_message_text(
            ...
            call.message.message_id,
            reply_markup=BotKeyboard.user_menu(
                {"username": db_user.username, "id": db_user.id, "status": "active"},
                view_user=True,
            ),
        )
