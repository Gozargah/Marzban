...
export function DataTable<TData extends UserResponse, TValue>({ columns, data, isLoading = false, isFetching = false, onEdit }: DataTableProps<TData, TValue>) {
  ...
  const ExpandedRowContent = useCallback(
    ({ row }: { row: any }) => (
            ...
            : table.getRowModel().rows?.length
              ? table.getRowModel().rows.map((row, index) => (
                  <React.Fragment key={row.id}>
                    <TableRow
                      className={cn(
                        'cursor-pointer border-b hover:!bg-inherit md:cursor-default md:hover:!bg-muted/50',
                    ...
                    {expandedRow === row.original.id && (
                      <TableRow
                        className={cn('border-b hover:!bg-inherit md:hidden', index >= visibleRows && 'opacity-0', 'transition-all duration-300 ease-in-out')}
                        style={{
                          transform: index >= visibleRows ? 'translateY(10px)' : 'translateY(0)',
                        }}
                      >
