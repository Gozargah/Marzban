        ...
        <Card dir={dir} className="group relative h-full w-full overflow-hidden rounded-lg border transition-all duration-300 hover:shadow-lg">
          ...
          <CardContent className="relative z-10 flex h-full flex-col justify-between p-4 sm:p-5 lg:p-6">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              <span
                dir="ltr"
                className={cn('text-xl font-bold transition-all duration-500 sm:text-2xl lg:text-3xl', isIncreased.online_users ? 'animate-zoom-out' : '')}
                style={{ animationDuration: '400ms' }}
              >
                <CountUp end={systemData?.online_users || 0} />
              </span>
              {isIncreased.online_users !== undefined && (
                <div className={cn('flex shrink-0 items-center text-xs', isIncreased.online_users ? 'text-green-500' : 'text-red-500')}>
