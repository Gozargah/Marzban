...
Если в логах **`Address already in use`** на **`0.0.0.0:53`**, другой процесс уже занял порт (не путать с **`127.0.0.53:53`** у `systemd-resolved` — это другой адрес). Найти, кто слушает 53:
...
Устраните конфликт (остановите лишний резолвер / освободите `:53`) или смените **`SMART_DNS_PORT`** и настройте проброс с внешнего DNS (сложнее для делегирования NS). Метрики нод при ошибке bind DNS по-прежнему опрашиваются поллером (после обновления панели).
